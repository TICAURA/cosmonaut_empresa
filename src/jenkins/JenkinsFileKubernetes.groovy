    ...
    stage("Compilacion") {
     ...
     stage("Push de imagen") {
        container('docker-registry'){
            script{
				sh "/kaniko/executor -f `pwd`/Dockerfile -c `pwd` --cache=true --destination=gcr.io/cosmonaut-uat/cosmonaut/empresa:$tag"
			}
        }
     }
        ...
        container('container-registry'){
            script{
		      ...
		      sh "kubectl  --insecure-skip-tls-verify get pods"
		       sh "sed -i 's/\$TAG/$tag/' src/jenkins/cosmonaut_empresa_deployment-qa.yml"
		       sh "cat src/jenkins/cosmonaut_empresa_deployment-qa.yml | kubectl --insecure-skip-tls-verify apply -f -"
